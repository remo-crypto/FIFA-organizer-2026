...
window.addEventListener('DOMContentLoaded', () => {
  ...
  announceAccessibility("FIFA Smart Stadium Operations Platform Loaded in English.");
});
...
// 8. Generative AI Operations Copilot Engine (Parser)
const copilotResponses = {
  "which gate is most congested?": {
    answer: "Currently, **Gate B** is showing the highest congestion on our sensors. Wait times have spiked to **24 minutes** due to ticket validation backlogs. Other entry points: Gate A (6 mins), Gate C (11 mins), Gate D (4 mins).",
    recommendation: "Deploy **4 floating volunteers** from Zone 3 to Gate B check-in lanes immediately. Activate the overhead signage at North Concourse to redirect 30% of arriving pedestrian streams to **Gate A**."
  },
  "predict crowd density for the next hour.": {
    answer: "Crowd intelligence forecasts a major density wave in **15 to 30 minutes** (approaching match kick-off). Attendance is expected to rise by **5,500 additional arrivals**.",
    recommendation: "Keep all turnstiles operating at maximum bandwidth. Pre-position medical responders near Gate B and Concourse Stand 12 to handle potential crowding congestion incidents."
  },
  "generate an evacuation plan.": {
    answer: "### EMERGENCY STADIUM EVACUATION PLAN\nEvacuation protocols are pre-indexed based on FIFA Safety Regulations. Trigger: **All Zones Evac**.\n1. **Acoustic Warning**: Broadcast emergency alarms in English, Spanish, and French.\n2. **Physical Exits**: Open all safety gates (A-D, North, West). De-energize turnstile locking pins.\n3. **Volunteers Roles**: Clear concourse corridors; guide fans away from escalators to safety plazas.",
    recommendation: "[CLICK TO EXECUTE ALARM] The system is in recommendation-only mode. Execute evacuation alerts through the physical command key switches."
  },
  "summarize today's operational issues.": {
    answer: "### OPERATIONAL ISSUES SUMMARY (Match Day 14)\n1. **Gate B Crowding** (08:31 UTC): Wait times peaked at 24 minutes. Current risk: **0.85**.\n2. **Medical Dispatch** (08:28 UTC): Responders deployed to Stand 104 - Row G for heat exhaustion.\n3. **Suspicious Baggage Alert** (08:20 UTC): Edge YOLO detected unattended bag at Stand 12. Resolved by security team: bag identified as lost jacket.",
  ...
  setTimeout(() => {
    ...
    Object.keys(copilotResponses).forEach(key => {
      ...
      if (normalized.includes(normKey) || normKey.includes(normalized)) {
        matched = copilotResponses[key];
      }
    });

    if (matched) {
      const responseHtml = `
        <p>${parseMarkdown(matched.answer)}</p>
        <div style="margin-top: 8px; border-top: 1px solid rgba(255,255,255,0.05); padding-top: 6px; font-size: 0.85rem;">
          <strong>AI recommendation:</strong> ${parseMarkdown(matched.recommendation)}
        </div>
      `;
      appendCopilotMessage(responseHtml, 'bot', true);
    } else {
      // General RAG Fallback response
      const fallbackText = `I have run a vector search in the **FIFA 2026 Stadium Operations Manual**. Regarding *"${rawQuery}"*, I suggest checking section 4.2 (Incident Management Protocols). 
      \n\nActive telemetry shows Gate A and D are stable, but Gate B has elevated congestion. If this requires dispatching volunteers, please use the REST API commands or query: *'Summarize today's operational issues.'*`;
      appendCopilotMessage(fallbackText, 'bot');
    }
  }, 800);
}
...
// Simple Helper to parse basic markdown (**bold**, ### headers)
...
function swapRightPanel() {
  ...
  if (rightPanelMode === 'copilot') {
    ...
    swapBtn.textContent = 'Show Copilot';
    title.textContent = 'Fan App Simulator';
    switchPhoneScreen(phoneActiveScreen);
  } else {
    rightPanelMode = 'copilot';
    copilotView.style.display = 'flex';
    fanAppView.style.display = 'none';
    swapBtn.textContent = 'Show Fan App';
    title.textContent = 'Operations Copilot';
  }
}
...
function simulateOCRTranslation() {
  alert("OCR Scanner Scan: Detected Physical Sign 'SALIDA DE EMERGENCIA' -> Translated to 'EMERGENCY EXIT' in English.");
}
...
function changeDashboardLanguage(langCode) {
  ...
  const textMatches = {
    en: "FIFA Smart Stadium Operations Platform Loaded in English.",
    es: "Plataforma de operaciones de estadios inteligentes cargada en español.",
    fr: "Plateforme d'opérations de stade intelligent FIFA chargée en français."
  };

  announceAccessibility(textMatches[langCode] || textMatches['en']);
  
  // Dynamic header translation mock
  const title = document.querySelector('.brand-name');
  if (title) {
    if (langCode === 'es') title.textContent = "Control de Estadios FIFA 2026";
    else if (langCode === 'fr') title.textContent = "Opérations du Stade FIFA 2026";
    else title.textContent = "FIFA 2026 Smart Stadium Command";
  }
}
